...
$\subset{x}{T}{R~x~a} "->" B$ ou $R$ est la relation bien fondée et $T$ le type de
l'argument de récursion ainsi on génera les obligations automatiquement
...
\section{La tactique \Program}
On peut utiliser notre contribution à l'aide des deux tactiques
\verb|Program| et \verb|Recursive Program|. 
...
type spécifié puis on réecrit le terme obtenu dans \CCI{}. On réecrit
...
\subsubsection{\texttt{Recursive Program}}
Cette deuxième permet d'écrire des définitions récursives, on va donc demander
plus d'informations à l'utilisateur lors de la définition:
$\texttt{Program}~\sref{name}~(a : `t_a) { \sref{wf}~R~(\sref{auto} `|
  \sref{proof} p)? } : ~`t := `a.$.
...
La tactique fait ensuite à peu près la même chose que \verb|Program|.
Le typage et la réecriture se font dans un contexte où la
...
un type de la forme $\Pi x : `t_a. R~x~a "->" `t$). 
