...
\begin{lemma}[Coercion de sortes]
  ...
  Si $\subimpl{`G}{e}{s}{T}$ ou $\timpl{`G}{e}{T}{s}$ alors $T
  \eqbr s$ et $e = \ctxdot$.
\end{lemma}
\begin{proof}  
...
\begin{proof}
  ...
  \begin{induction}
    ...
    \begin{itemize}
      ...
      \ref{subti-eqb-coercion-eqbres-id}, on a
    ...
    T'[u/x]$. 
    Par le lemme \ref{subti-eqb-coercion-eqbres-id}, on
    ...
    \[\begin{array}{ll}
      ...
      {\hat{=}}(\pi_F[\ip{f}{\GD}])~(c_e[\ip{e}{\GD}])\ipux 
...
\begin{lemma}[Stabilité par affaiblissement des coercions]
  ...
  Si $`r : `D \sub `G$, $\subimpl{`G}{c}{T}{T'}$ et $\ip{T'}{`D} `=
  \ip{T'}{`G}[`r]$, alors $\subimpl{`D}{c'}{T}{T'}$ et $c' `= c[`r]$.
\end{lemma}
...
\begin{proof}
  ...
  \begin{induction}
    ...
    lemme puisque si $\muimpl{F} `= \Pi y : A.B$, alors clairement 
    ...
    Ce diagramme commute, il existe donc une coercion $c_{E',A} `=
    c_e[`r] `o `a_e : E' \sub A$.    
    Par le lemme \ref{narrowing-i-coercion} on a donc
    $\subimpl{`D, x : E'}{c_2'}{B'}{B}$ avec $c_2' `=
    c_2[(c_e[`r])[`a_e[x]]/x]$.
    ...
    c_2'[\ip{e}{`D}/x] `= c_2[(c_e[`r])[`a_e[e]]/x]\]. 
