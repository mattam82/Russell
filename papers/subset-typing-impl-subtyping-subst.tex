...
\begin{proof}
  ...
  \begin{induction}
    ...
    induction: $\subimpl{`D, x : C}{c_2'}{B}{D}$ avec $c_2' \eqb c_2[`s]$.
    
    On peut alors appliquer \irule{SubProd} pour obtenir:
    \[\subimplhnf{`D}{c' = \lambda f : \ip{\Pi x : A.B}{`D}.~\lambda x :
    ...
    \begin{prooftree}
      ...
      {$\subimplconcl{`G}{t}{\letml~(x, y) = t~\inml~(c_1~x, c_2~y)}
    ...
    \[\begin{array}{ll}
      \firsteq{
        ...
        \step{Application de l'hypothèse de récurrence}
        {\eqbe}{\lambda t : \ip{\Sigma x : A.B}{`D}.~\letml~(x, y) = t~\inml~(c_1[`r]~x, c_2[`r]~y)}
   ...
   Par induction, on a $\subimplhyp{`D}{c[`r]}{T}{U}$. On peut donc dériver:
   \[\subimplconcl{`D}
   {t}{\elt{\ip{U}{`D}}
     {\ip{(\lambda x : U.P)}{`D}}
     {(c[`r]~t)}
   ...
   \begin{equations}
     ...
     \step{Définition de l'interprétation}
     {\eqb}{\ipG{`G}[`r], t : \ip{T}{`G}[`r] \vdash \ip{P}{`G, x : U}[c[`r]~t/x][`r]}

     \step{Hypothèse de récurrence}
     {\eqb}{\ipG{`D}, t : \ip{T}{`D} \vdash \ip{P}{`G, x : U}[c[`r]~t/x][`r]}

     \step{Hypothèse de récurrence, $\ip{P}{`G, x : U}[`r] = \ip{P}{`D,
         x : U}$ et $x `; `r$}
     {\eqb}{\ipG{`D, t : T} \vdash \ip{P}{`D, x : U}[c[`r]~t/x]}
...
\begin{proof}
  ...
  \begin{induction}
    ...
    Ce diagramme commute, il existe donc une coercion $c_{E',A} \eqbe
    c_e[`r] `o `a_e : E' \sub A$.    
    Par le lemme \ref{narrowing-i-coercion} on a donc
    $\subimpl{`D, x : E'}{c_2'}{B'}{B}$ avec $c_2' \eqbe
    c_2[c_e[`r]~(`a_e~x)/x]$.

    Par le lemme \ref{subti-coercion-subst} avec $`D \typec e : E'$ on obtient:
    $\subimpl{`D}{c_2''}{B'[e/x]}{B[e/x]}$ avec $c_2'' \eqbe
    c_2'[\ip{e}{`D}/x] \eqbe c_2[c_e[`r]~(`a_e~e)/x]$.    
    ...
    \[\begin{array}{ll}
      ...
      {\eqbe}{(\pi_F[`r]~(`a_f~\ip{f}{`D}))~(c_e[`r]~(`a_e~\ip{e}{`D}))}
