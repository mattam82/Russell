...
\begin{proof}
  ...
  \begin{induction}
    ...
    On a $\talgo{`G}{\Pi y : A'.B'}{s}$, donc par inversion,
    $\talgo{\GD}{A'}{s_1}$. On applique l'hypothèse d'induction
    pour obtenir $\ip{A'[u/x]}{\Gr} `= \ip{A'}{\GD}\ipux$.
    On a donc bien $c' `= c\ipux$. 
    \Problem{Ca n'est pas une sous dérivation}
    ...
    \begin{prooftree}
      ...
      {$\GD \seq t : T$}
      {$\GD \seq v : U'$}
      {$\GD \seq \Sigma y : T.U : s$}
      {$\GD \seq U' : s \quad U' \suba U[t/y]$}
      {$\GD \seq \pair{\Sigma y : T.U}{t}{v} : \Sigma y : T.U$}
      {}
    \end{prooftree}
    ...
    \begin{prooftree}
      ...
      {$\Gr \seq t[u/x] : T[u/x]$}
      {$\Gr \seq v[u/x] : U'[u/x]$}
      {$\Gr \seq (\Sigma y : T.U)[u/x] : s$}
      {$\Gr \seq U'[u/x] : s$}
      {$U'[u/x] \suba U[u/x][t[u/x]/y]$}
      {$\Gr \seq (\pair{\Sigma y : T.U}{t}{u})[u/x] : (\Sigma y : T.U)[u/x]$}
      {}
    \end{prooftree}
    
    On a $U[u/x][t[u/x]/y] = U[t/y][u/x]$ puisque $y `; \freevars{u}$.
    Ici on a une coercion $\subimpl{\GD}{c}{U'}{U[t/y]}$. Par induction
    on obtient $\subimpl{\Gr}{c'}{U'[u/x]}{U[t/y][u/x]}$ avec $c' `= c\ipux$.

    \begin{equations}
      \firsteq{\ip{\pair{\Sigma x : T.U}{t}{v}}{\GD}\ipux}
      
      \step{Définition de l'interprétation}
      {\eqdef}{\pair{\ip{\Sigma x :
            T.U}{\GD}}{\ip{t}{\GD}}{c[\ip{v}{\GD}]}\ipux}
      
      \step{Application de l'hypothèse de récurrence}
      {`=}{\pair{\ip{(\Sigma x :
            T.U)[u/x]}{\Gr}}{\ip{t[u/x]}{\Gr}}{c'[\ip{v[u/x]}{\Gr}]}}

      \step{Définition de l'interprétation}
      {\eqdef}{\ip{(\pair{\Sigma x : T.U}{t}{u})[u/x]}{\Gr}}
    \end{equations}
...
\begin{proof}
  ...
  \begin{induction}
    ...
    Par transitivité de la coercion, on a $\subimpl{`D}{`a_{S'} `= \mu_S[`r]
      `o `a_t}{S'}{\Sigma x : T.U}$.
    On en déduit qu'il existe $T',U'$ tels que $\muimpl{S'} = \Sigma x :
    ...
    avec $`b `o \mu_{S'} `= \mu_S `o `a_t$.

    Soit $`a = c_1$, $\subimpl{`D}{c_1}{T'}{T}$.
