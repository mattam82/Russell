...
\begin{lemma}[Sous-typage bien sorté]
  ...
  Si $U \subd T$ alors $`G \typed U : s$ si et seulement si
  $`G \typed T : s$.
\end{lemma}
...
\TIC{$`G \seq t : T$}
\DP
par une application de la subsumption:
\AXC{\vdots}
\UIC{$`G \seq T : s$}
\AXC{\vdots}
\UIC{$`G \seq t : U$}
...
\TIC{$`G \seq t : T$}
\DP
et vice-versa. Les systèmes obtenus en incluant \ConvRule{} ou
\SubConvRule{} sont donc équivalents. 
...
\begin{lemma}[Conservation de la conversion par sous-typage]
  \label{conv-sub}
  Si $`G \typed t : T$, $T \eqbi U$ alors $`G \subta t : T \sub U$.
\end{lemma}
...
\begin{lemma}[Inversion du sous-typage]
  ...
  Si $`G \subta t : \Pi x : T.U \sub \Pi x : V.W$ 
  alors $t = \lambda x : T. M$ et $`G, x : T \typea M : U$.
