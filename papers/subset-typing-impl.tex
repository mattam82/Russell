...
\begin{lemma}[Coercion et conversion]
  \label{coercioni-conversion}
  Si $`G \typea S,T,U : s$, $S \eqbi T$ et $`G \typec
  c : T \sub U$ alors $\typec c' : S \sub U$ avec $c \eqbi c'$.
\end{lemma}
...
\begin{proof}
  ...
  forme:
  \begin{prooftree}
    \AXC{$`G \typec c_1 : A \suba B$}
    \AXC{$`G \typec c_2 : B \suba C$}
    \BIC{$`G \typec c_2 `o c_1 : A \suba C$}
  \end{prooftree}
...
\begin{proof}
  ...
  On omet les environement passés à $\coerce$ s'ils sont évidents.
