...
\subset{x}{\nat}{P}.\listml~n$ puisque $\subset{x}{\nat}{P} \suba
...
\item $`q, x = c x : `G', x : T' "->" `D', x : U'$ 
  si $`q : `G' \suba `D'$, $`G' \typec c : T' \subi U'[`q]$ soit 
  $`G', x : T' \typec c~x : U'[`q]$.
\end{itemize}
\begin{proposition}
  Si $T \suba U$ et $`G \suba T : s$ et $`D \suba U : s$ et $`G \suba
  `D$ alors on a $`z : `G' "->" `D'$ et $`p : `D' "->" `G'$ et
  $`G' \typec c_{`G} : T' \subi U'[`z]$ et $`D' \typec c_{`D} : T'[`p]
  \subi U'$.
\end{proposition}

Ainsi, à l'application on a la preuve que $`G' \typec c : U' \subi V'$
puisque la fonction de $`D$ à $`D$ est l'identité.
Il est à noter que les deux définitions de $`z : `G' "->" `D'$ et 
$`D' \typec c : T' \subi U'[`z]$ sont mutuellement récursives.
