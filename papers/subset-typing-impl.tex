...
\subset{x}{\nat}{P}.\sref{list~n}$ puisque $\subset{x}{\nat}{P} \suba
\nat$ et $\listml~n \eqbi \listml~n$.
Si l'on réecrit la dérivation vers \CCI{}, on n'aura plus les même types
...
\begin{prooftree}
  ...
  \UIC{$`G' \typec (\lambda x.x) `o \pi_1 : \subset{x}{\nat}{P} \subi \nat$}
...
correspondant au coercions appliquées dans la réecriture. 

Nous avons de bonnes raison de penser que la traduction fonctionne: le
système \PVS{} repose sur système similaire depuis des années, notre
réecriture fonctionne en pratique et enfin, on voit bien que le système
