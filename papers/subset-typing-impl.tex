...
\begin{figure}
  \[\begin{array}{lcll}
    ...
    & \\
    \ip{f~u}{`G} 
    & = & \letml~(\pi, \Pi x : V.W) = \muimpl(\typeafn{`G}{f})~\inml & \\
    & & \letml~c = \sref{coerce}~\typeafn{`G}{u}~V~\inml & \\
    & & (\pi~\ip{f}{`G})~(c~\ip{u}{`G}) & \\
    & \\
    \ip{\Sigma x : T.U}{`G} 
    ...
    & = & \letml~t' = \ip{t}{`G}~\inml & \\
%    & & \letml~U' = \ip{U}{`G, x : \type(t)}~\inml& \\
    & & \letml~u' = \ip{u}{`G}~\inml & \\
    & & \letml~c = \sref{coerce}~\typeafn{`G}{u}~U[t/x]~\inml & \\
    & & (t', c~u') & \\
    & \\
    \ip{\letml~(x,y) = t~\inml~v}{`G} 
    & = & \letml~t' = \ip{t}{`G}~\inml & \\
    & & \letml~(\pi, \Sigma x : T.U) = \muimpl(\typeafn{`G}{t})~\inml & \\
    & & \letml~v' = \ip{v}{`G, x : T, y : U}~\inml & \\
    & & \letcci~(x, y) = t'~\inml~v' & \\
    & \\
    \ip{\mysubset{x}{U}{P}}{`G}
    & = & \letml~U' = \ip{U}{`G}~\inml & \\
    & & \letml~P' = \ip{P}{`G, x : U}~\inml & \\
    & & \mysubset{x}{U'}{P'}
  \end{array}\]
