...
\begin{frame}[t]
  ...
  \uncover<3->{
    \begin{block}{Further refinements}
      ...
      \item<4-> Add inequalities for intersecting patterns ;
      \item<5-> Generalized to dependent inductive types.
      \end{itemize}
    \end{block}}
  ...
  \[ \begin{array}{ll}
    ...
    "=>" t_2 \\\only<3>{`| 0 "=>" & \uncover<2->{\funml{}~H : 0 = \eltpit{(e)} "=>"} t_2 \\}
  ...
  \only<5->{
    Let $e : \vector{n}$:
    \[ \begin{array}{l}
      \matchml~e~\uncover<6->{\asml~t~\inml~\vector{n'}}~\returnml~\uncover<6->{n' = n "->" t "~=" e "->"} T~\withml \\
      `| \vnil "=>" \uncover<6->{\funml{}~(H : 0 = n) (Hv : \vnil "~=" e) "=>"} t_1 \\
      `| \vcons~x~n'~v' "=>" \uncover<6->{\funml{}~(H : S~n' = n) (Hv :
        \vcons~x~n'~v' "~=" e) "=>"} t_2 \\%
    \enml \uncover<6->{(\coqid{refl\_equal}~n) (\coqid{JMeq\_refl}~e)}
  \end{array}\]}
...
\begin{frame}[t]
  ...
  \small{
  \coqdockw{Inductive} \coqdocid{fingertree} (\coqdocid{A} : \coqdockw{Type}) \only<2->{(\coqdocid{measure} : \coqdocid{A} \ensuremath{\to} \coqdocid{v})} : \only<2->{\alert{\coqdocid{v}} \ensuremath{\to}} \coqdockw{Type} $\coloneqq$\coqdoceol
  \coqor  \coqdocid{Empty} : \coqdocid{fingertree} \coqdocid{A} \only<2->{\coqdocid{measure} \alert{$\varepsilon$}}\coqdoceol
  \coqor  \coqdocid{Single} : \ensuremath{\forall} \coqdocid{x} :
  \coqdocid{A}, \coqdocid{fingertree} \coqdocid{A} \only<2->{\coqdocid{measure} (\alert{\coqdocid{measure} \coqdocid{x}})}\coqdoceol
  \coqor  \coqdocid{Deep} : \ensuremath{\forall} (\coqdocid{l} : \coqdocid{digit} \coqdocid{A}) \only<2->{(\alert{\coqdocid{m} : \coqdocid{v}})},\coqdoceol
  \coqdocindent{1.00em}
  \coqdocid{fingertree} (\alert<1>{\coqdocid{node} \coqdocid{A}} \only<2->{ \coqdocid{measure}})
  \only<2->{(\alert<2->{\coqdocid{node\_measure} \coqdocid{A} \coqdocid{measure}}) \alert<2->{\coqdocid{m}}} \ensuremath{\to} \ensuremath{\forall} \coqdocid{r} : \coqdocid{digit} \coqdocid{A}, \coqdoceol
  \coqdocindent{1.00em}
  \coqdocid{fingertree} \coqdocid{A} \only<2->{\coqdocid{measure}
    (\alert{\coqdocid{digit\_measure} \coqdocid{measure} \coqdocid{l}
...
\begin{frame}
  ...
  \begin{block}{The monoid and measure: executable semantics}
    \scriptsize{\coqdockw{Definition} \coqdocid{below} \coqdocid{i} := \{ \coqdocid{x} : \coqdocid{nat} | \coqdocid{x} $<$ \coqdocid{i} \}.\coqdoceol
  ...
  \begin{block}{Specialization}
    \scriptsize{\coqdockw{Definition} \coqdocid{seq} (\coqdocid{x} : \coqdocid{v}) := \coqdocid{fingertree} \coqdocid{seqMonoid} \coqdocid{measure} \coqdocid{x}.\coqdoceol
