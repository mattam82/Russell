...
\begin{frame}[t]
  ...
  \only<5->{
    Let $e : \vector{n}$:
    \[ \begin{array}{l}
      \matchml~e~\uncover<6->{\asml~t~\inml~\vector{n'}}~\returnml~\uncover<6->{\alert{n' = n} "->" t \alert{"~="} e "->"} T~\withml \\
      `| \vnil "=>" \uncover<6->{\funml{}~(H : \alert{0 = n}) (Hv : \vnil \alert{"~="} e) "=>"} t_1 \\
      `| \vcons~x~n'~v' "=>" \uncover<6->{\funml{}~(H : \alert{S~n' = n}) (Hv :
