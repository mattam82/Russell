...
\begin{proof}
  ...
  du système déclaratif, excepté pour la règle \irule{SubHnf}.
...
\begin{proof}
  Par le simple fait que si $S = \subset{x}{U}{P}$ alors $S : \Set$ et
...
\begin{proof} 
  ...
  \begin{induction}[subtyping-decl]
    ...
    \irule{SubHnf}, $S \sub U$.
    
    \case{SubProd}\quad
    \begin{prooftree}
      \AXC{$X' \sub X$}
      \AXC{$Y \sub Y'$}
      \BIC{$\Pi x : X.Y \sub \Pi x' : X'.Y'$}
      \AXC{$\Pi x' : X'.Y' \sub U$}
      \BIC{$\Pi x : X.Y \sub U$}
    \end{prooftree}
    
    Si $\Pi x' : X'.Y' \sub U$, alors il existe $S$, $T$, tel que $\mualgo(U)
    = \Pi y : S.T$ et $S \sub X'$, $Y' \sub T$.
    Par induction (les dérivations sont plus petites que la dérivation
    $\Pi x' : X'.Y' \sub U$), $S \sub X$ et $Y \sub T$ donc $\Pi x : X.Y
    \sub \Pi y : S.T$ 
    et enfin $\Pi x : X.Y \sub U$ par le lemme \ref{coercion-mu}.
    ...
    \begin{prooftree}
      \AXC{$X \sub X'$}
      \AXC{$Y \sub Y'$}
      \BIC{$\Sigma x : X.Y \sub \Sigma x' : X'.Y'$}
      \AXC{$\Sigma x' : X'.Y' \sub U$}
      \BIC{$\Sigma x : X.Y \sub U$}
    \end{prooftree}
    
    Si $\Sigma x' : X'.Y' \sub U$, alors il existe $S$, $T$, tel que $\mualgo(U)
    = \Sigma y : S.T$ et $X' \sub S$, $Y' \sub T$.
    Par induction, $X \sub S$ et $Y \sub T$ donc $\Sigma x : X.Y \sub
    \Sigma y : S.T$ et enfin $\Sigma x : X.Y \sub U$ (lemme \ref{coercion-mu}).
...
l'implémentation puisque le système est confluent. \irule{SubHnf}
...
\begin{proof}
  ...
  \begin{induction}
    ...
    n'apparaît pas dans $U$. On a donc bien $`G, `D[u/x] \typea t[u/x] = u :
    T[u/x] = U$. 
