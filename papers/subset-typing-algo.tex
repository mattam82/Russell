...
\begin{lemma}[Conservation des sortes par $\mu$]
  \label{mu-sorts}
  Si $\talgo{`G}{S}{s}$ alors $\talgo{`G}{\mu~S}{s}$
\end{lemma}
...
\begin{proof}
  \begin{induction}[typing-algo]
    ...
    \irule{Coerce}.
    Par le lemme \ref{correct-coercion} appliqué à $\talgo{`G}{U}{V}$
...
\begin{proof}
  ...
  \begin{induction}
    ...
    $\talgo{`G, x : V}{Y, W}{s}$ de taille inférieure ou égale aux
    dérivations de $\talgo{\hnf{T}}{\hnf{U}}{s}$.
...
\begin{proof}
  ...
  \begin{induction}
    ...
    $\talgo{`G, `D[u/x]}{\pi_2~t[u/x]}{U[u/x][\pi_1~t[u/x]/y] =
      U[\pi_1~t/y][u/x]}$ car $y `; \freevars{u}$.
    ...
    Par induction et application de \irule{SumDep}, en utilisant la transitivité:
    \begin{prooftree}
      \SumDepAFullWide[\Gr]{y}{t[u/x]}{v[u/x]}{T[u/x]}{U[u/x]}{s}{T'[u/x]}{V'[u/x]}
    \end{prooftree}
...
\begin{lemma}[Substitutivité du typage avec coercion et affaiblissement]
...
\begin{proof}
  ...
  \begin{induction}
    ...
    \begin{induction}
      \caseother{Si $y = x$} alors $T = V$. On a $t[u/x] = u$ donc $T' =
      U$. Par hypothèse $U \suba V$ donc $T' \suba T[u/x]$ triviallement
      ($x$ n'apparait pas dans $V$).
      
      \caseother{Sinon} $t[u/x] = y$ donc $T' = T[u/x]$ et on a $T' \suba T[u/x]$
    ...
    \casenil Les autres cas passent aisément par induction.
  \end{induction}
