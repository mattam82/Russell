...
\begin{lemma}[Coercion et $\mualgo$]
  ...
  \item Si $\Sigma x : X.Y \sub U$ alors $\mualgo(U) = \Sigma x : X'.Y'$ et $X \sub
    X'$, $Y \sub Y'$.
  \item Pour tout $U$, $U \sub \mualgo(U)$.
...
\begin{proof}
  ...
  Dans le dernier cas, de gauche a droite on construit la dérivation en
...
\begin{lemma}[Coercion et conversion]
  ...
  Si $`G \type S,T,U : s$, $S \eqbi T$ et $T \sub U$ alors $S \sub U$
\end{lemma}
...
\begin{proof} 
  ...
  \begin{induction}[subtyping-decl]
    ...
    = \Pi~S~T$ et $S \sub X'$, $Y' \sub T$.
    Par induction (les dérivations sont plus petites que la dérivation
    $\Pi~X'~Y' \sub U$), $S \sub X$ et $Y \sub T$ donc $\Pi~X~Y \sub \Pi~S~T$ 
    et enfin $\Pi~X~Y \sub U$ par le lemme \ref{coercion-mu}.
    ...
    Si $\subset{x}{V}{P} \sub U$ alors $\mualgo(U) = \mualgo(V)$.
    Comme $S \sub V$ on a $S \sub \mualgo(V)$ par une dérivation plus
    petite ou égale, donc par induction, $S \sub \mualgo(U)$. Donc $S \sub U$.
...
algorithmique avec \irule{Coerce}.
On montre ensuite que toute dérivation de ce système se réecrit dans
une dérivation du système algorithmique sans la règle \irule{Coerce}.
En composant les deux preuves on
obtient que toute dérivation du système déclaratif a une dérivation dans
le système algorithmique (sans \irule{Coerce} donc).

La première étape est directe: les deux systèmes ont les mêmes règles,
il y a juste \irule{App} et \irule{LetSum} qui ont changé.

\begin{theorem}
  \label{typed-typea-coerce}
  Si $`G \typed t : T$ alors $`G \typea t : T$ où $\typea$ inclut la
  règle \irule{Coerce}.
\end{theorem}
\begin{proof}
  \begin{induction}{typing-decl}
    ...
    \typenva
    On suppose qu'on a $`G \type f : \Pi x : V.W$ et $`G \type v : V$. 
    on peut donc dériver 
    \begin{prooftree}
      \TAX{App}
      {$`G \seq f : \Pi x : V.W \quad \mualgo(\Pi x : T.U) = \Pi x : T. U$}
      {$`G \seq v : V \quad `G \seq V, V : s$}
      {$V \suba V$}
      {$`G \seq (f u) : W [ u / x ]$}
      {}
    \end{prooftree}
    ...
    \typenva
    De même on peut dériver:
    \begin{prooftree}
      \TAX{LetSum}
      {$`G \seq t : \Sigma x : T.U$}
      {$\mualgo(\Sigma x : T.U) = `S x : T. U$}
      {$`G, x : T, y : U \seq v : V$}
      {$`G \seq \letml~(x, u) = t~\inml~v : V$}
      {}
    \end{prooftree}
    
  \item[- Autres règles:] identiques.
  \end{induction}
\end{proof}
...
\begin{proposition}[Passage de la coercion à l'application]
  ...
  coercion vers une dérivation $`G \typea t : U$ avec $U \suba T$.
\end{proposition}
...
\begin{proof}
  ...
  \begin{induction}
    ...
    \case{WfVar} 
    Par induction $`E s' `G', `G' \typea A : s' \sub s$ (cela implique $s'
    = s$). On applique \irule{WfVar} pour obtenir $\typewf `G, x : A$.
    
    \case{PropSet} Trivial.

    \case{Var} Par induction $\typewf `G'$ et $x : A `: `G'$ , direct par
    \irule{Var}.
    
    \case{Prod} 
    Direct par induction et le fait qu'un sorte ne peut être en relation
    qu'avec elle même (on ne peut introduire le type $\subset{x}{s}{P}$
    pour une sorte $s$).
        
    \case{Abs} \quad
    Par induction $`E U', `G, x : T \typea M : U' \suba U$.
    Or cela implique $`G, x : T \typea U' : s$, donc on a le produit 
    ...
    \begin{prooftree}
      ...
      \UIC{$S \sub V$}
      \BIC{$`G \seq t : V$}
      \AXC{$\mualgo(V) = \Sigma x : T.U$}
      \AXC{\vdots}
      \UIC{$`G, x : T, u : U \seq v : V $}
      \TIC{$`G \seq \letml~(x, u) = t~\inml~v : V$}
    \end{prooftree}
    
    Par le lemme \ref{beta-mu}, on a $\mualgo(S) = \Sigma x : T'.U'$,
    ...
    Par le lemme \ref{beta-mu} on a $\mualgo(T) = \Pi x : V'.W'$ avec
...
\begin{proof}
  \begin{induction}[typing-decl]
  \item[\irule{WfEmpty},\irule{WfVar},\irule{PropSet},\irule{Var},\irule{Prod},\irule{Abs},
    \irule{LetIn}, \irule{Sigma}, \irule{Subset}:] règles
    inchangées. Dans chaque cas la dernière règle appliqué n'est pas
    \irule{Coerce} mais \irule{Coerce} peut apparaître dans la
    dérivation. Or on sait par \ref{subsum-elim} que l'on peut réecrire
    toute dérivation utilisant \irule{Coerce} vers une dérivation ne
    l'utilisant qu'à sa racine (soit vers une dérivation de $`G \typea t
    : U$ avec $U \suba T$. \TODO{fix}
