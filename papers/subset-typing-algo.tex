...
typage que pour la correction. On va donc rafiner ces systèmes dans
...
éloigné des originaux, néanmoins nous allons montrer qu'ils sont
...
pour aboutir d'une part au théorême de complétude qui montre qu'on peut dériver les
...
\begin{proof}  
  ...
  \begin{induction}[subtyping-decl]
    ...
    Comme $S \sub V$, par induction, $S \sub \mu(U)$ donc
    $S \sub U$.
...
algorithmique. En particulier, on a utilisé la fonction $\mu0$ de \PVS{}
...
\begin{fact}[Inversion du jugement de bonne formation]
  ...
  Si $\typewf `G, x : U$ alors il existe $s$, $`G \type U : s$ et $s `: \{ \Set, \Prop, \Type(i) \}$.
\end{fact}
