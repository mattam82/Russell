...
\begin{lemma}[Complétude du typage]
  ...
  $`G \typed t : T "=>" `E U, `G \typea t : U \sub T$
\end{lemma}

\begin{proof}
  On a montré que l'on pouvait éliminer la règle coerce (proposition
  \ref{subsum-elim}), on a donc une dérivation du système algorithmique
  correspondante à chaque dérivation du système déclaratif. On peut donc
  raisonner sur des dérivations où 

  \begin{induction}[typing-decl]
  \item[\irule{WfEmpty},\irule{WfVar},\irule{PropSet},\irule{Var},\irule{Prod},\irule{Abs},
    \irule{LetIn}, \irule{Sigma}, \irule{Sum}, \irule{Subset}:] règles
