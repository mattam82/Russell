...
\begin{code}
  Definition tail (l : list A) : option A :=
    match l with
      hd :: tl => Some tl
    | [] => None
    end.
...
does:
\begin{code}
  Definition tail (l : list A) : forall p : l <> [] -> 
...
constructing objects of a subset type. Using \Russell, we would write:
\begin{code}
...
The specification shows that we're defining a partial function and
shows the relation betwenn input and ouput, using a dependent type, yet the
...
\begin{enumerate}
\item The list $tl$ has the property \verb|exists a, a :: tl = hd :: tl|
  and
\item In the context where
  \id{l} is a non-empty list and $l = []$, the list $[]$ has the property
...
\begin{code}
  Definition head \{ l : list A | l <> 0 \} : A :=
    match l with
    | hd :: tl -> hd
    | [] -> _
    end.
...
$l \neq 0$, but we may not have any placeholder element of type $A$.  
