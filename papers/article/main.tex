...
\newenvironment{code}{\begin{alltt}\vspace{-1em}}{\vspace{-1em}\end{alltt}}
% Caml2Tex
...
simple terms to represent complex types, we immediately get
undecidability of type-checking, as the terms do not give enough
information. Consider for example the function \id{head} which returns
the first element of a list. In \ML{}:
\begin{code}
  let head = function hd :: tl -> hd | [] -> assert(false)
\end{code}

This function is partial, its domain is reduced to non-empty lists. In
\Coq, we would rather write the following:
\begin{code}
  Definition head (l : list A) : option A :=
    match l with
      hd :: tl => Some hd
    | [] => None
    end.
...
non-empty lists to be more faithful to the original \ML code, using a
richer specification: 
\begin{code}
  Definition head (l : list A) : forall p : l <> [] -> A.
\end{code} 
Now the definition's body requires some heavy plumbing of the code which is 
not affordable when programming. We propose a solution to overcome this difficulty, based on the \ps{}
\cite{Shankar&Owre:WADT99} technique of \PVS{}
\cite{PVS-Semantics:TR}. It is a weak, \emph{decidable} type-checking
procedure along with an elaboration into partial \Coq terms. Using
\Russell, we would write:
\begin{code}
  Definition head \{ l : list A | l <> [] \} : A := 
  match l with
    hd :: tl -> hd
  | [] -> _
  end
...
The specification shows that we're defining a partial function yet the
code remains as simple as the \ML{} definition. This is only possible
because we leave the proof in the second branch. After type-checking,
the proof assistant will ask the user to prove that in the context where
\id{l} is a non-empty list and $l = []$, we can create an object of type
$A$ (which should be obvious as the context is contradictory).

This solution also provides % PVS:Language,
facilities to express properties with a more mathematical flavor using subsets,
...
The remainder of the article is organized as follows: in section \ref{sec:Russell} we present a
type system based on \CIC, which integrates subset ``subtyping'', and
we show how it relates to \CIC{} by means of translations between
judgments of the two systems in section \ref{sec:Translation}. Then, we present a prototype
...
want to use an operation defined on a subset, e.g: $f : \{ x : `N `| P \} "->"
...
To formalize this idea in \Coq, we simply weaken the type system so that
...
judgment $`G \typed t : T$ means $t$ is a well-typed term of type $T$
in environment $`G$.  In \Russell, the conversion rule is replaced by a
new subsumption rule \irule{Subsum} (figure
\vref{fig:typing-decl-rules}) which will implement the subset
equivalence. The judgment $`G \typed T \subd U : s$ means $T$ is
...
by the functional relation $(s_1, s_2, s_2)$. We allow products $\Pi x :
...
Likewise, we need to restrict the system so that it becomes property
irrelevant for the propositions appearing in subset types. Indeed, in
\PVS{} you can't manipulate proof terms, so you have no way to create a
term of type $0 \neq 0$ from the judgment $0 : \mysubset{x}{`N}{x \neq 0}$, but
in a classical Calculus of Constructions with sigma types or inductive
types, the subset type is a first class object defined with proper
introduction and elimination terms (figure \vref{fig:cci-subset}). What
we need to do is forbid the use of these terms in \Russell{} terms,
leaving only the type former (figure \vref{fig:typing-decl-rules}).
...
of $x$ and by $\hnf{x}$ it's head normal form.
...
include the usual $\beta\pi$-conversion directly in the judgment.
...
judgment. We use a judgmental equality here, which will be refined by
the usual conversion relation later. The \irule{SubSym} and
\irule{SubTrans} rules ensure that our judgment constructs an \emph{equivalence} and has
...
still get the same judgments because we have symmetry. However, it will
...
Our algorithmic judgment $`G \typea t : T$
...
\end{paragraph}
\begin{paragraph}{}
  The next thing to do is to construct a decidable judgment for
...
\begin{theorem}[Completeness]
  If $`G \typed t : T$ then $`G \typea t : T'$ and $`G \typea T' \suba T : s$.
...
the identity on \Russell terms. Indeed, the second projection of a
subset element can only appear in the second component of a subset
...
metavariables) in the target calculus ($\typeq$ denotes the new system's typing judgment):
\begin{prooftree}
  \UAX{}
  {$`G \typeq P : \Prop$}
  {$`G \typeq ?_{P} : P$}
...
the judgment which build a coercion given two types (we denote it $\coerce{`G}{T}{U}$). 
The side-conditions of the rules have not changed, so we omit them for
improved readability. The judgment $\subimpl{`G}{c}{T}{U}$ builds a
...
derivation but the resulting object will be a well-typed \Coq{} term. We
...
in defining a judgment $x : A \type y : B$ which reads: $A$ is
...
\interpfigs[Interpretation of terms]
...
$\beta\pi$-equivalence. 
Indeed, if our algorithmic system has transitivity and symmetry of
coercions, we expect that the derived coercions will inherit these
properties, for example:
\begin{proposition}[Symmetry of coercions]
...
\begin{example}[Symmetry for products]
...
\ccqeqfig[Equational theory of \CICq]
...
available (in French) and a mechanically checked proof is currently developed.
...
of coercion to contexts next and show stability of judgments under
...
equivalence is conserved by interpretation and finally that our
interpretation is correct.

\section{The \Program{} tactic}
\label{sec:Program}
