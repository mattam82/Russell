...
proof term when calling the function.
...
constructing objects of a subset type. Using our new language \Russell, we would write:
\begin{code}
...
However, in a classical Calculus of Constructions with sigma types or inductive
types, the subset type is a first class object defined with proper
introduction and elimination terms (figure \vref{fig:cci-subset}). These
are not compatible with our subtyping, so we need to forbid the use of
these terms in \Russell{} terms, leaving only the type former (figure
\vref{fig:typing-decl-rules}). With this restriction, when we erase subset types
from \Russell terms, leaving only the supports, we get a valid
\CC term, hence $`_$ is not provable in \Russell.
...
\begin{proof}
  ...
  \item Immediate using a thinning lemma.
  \end{enumerate}
\end{proof}
\end{paragraph}
...
\begin{theorem}[Completeness]
  If $`G \typed t : T$ then there exists $T'$, $`G \typea t : T'$ and $`G \typea T' \suba T : s$.
