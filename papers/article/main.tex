...
Nordstr\"om et al. \cite{progmartinlof} and subset types in NuPRL.
...
the usual conversion relation later. The \irule{SubTrans} rule ensure
...
in context $\ipG{`G}$. We use the \lng{} types to drive the
...
gives:
$p~x =_{P~x} \ip{q~x}{`G} = (\lambda x : B. q~d[x]) c[x] =
q~d[c[x]]$. The right hand side has type $P~d[c[x]]$ which must be
convertible with $P~x$ for the whole term to be well-typed. This
indicates that coercions must be unique up to eta conversion and eta
conversion must be included in the definitional equality. For
