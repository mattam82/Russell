...
\newenvironment{code}{\begin{alltt}\vspace{-1em}}{\end{alltt}\vspace{-1em}}
...
simple terms to represent complex types, we immediatly get
...
not afordable when programming. We propose a solution to overcome this difficulty, based on the \ps{}
...
The specification shows that we're definining a partial function yet the
...
judgements of the two systems in section \ref{sec:Traduction}. Then, we present a prototype
...
% essentially because of the proof terms (hence the use of a judgement
% instead of a function for checking subtyping), but we have also extended
% it by . 
...
our languages subtyping mechanism.
...
term of type $0 \neq 0$ from the judgment $0 : \mysubset{x}{`N}{P}$, but
...
coerces an object of type $\ip{`G}{T}$ to an object of type $\ip{`G}{U}$
...
\interpfigs[Interpretion of terms]
