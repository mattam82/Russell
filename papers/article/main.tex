...
attach a property to the result to express more precisely what \id{tl}
does:
\begin{code}
  Definition tl (l : list A) : (l <> []) -> 
...
a term is formalised by the \Prop/\Set{} sorts. In \Russell, we have
...
encode them as $\Pi x : \{ \_ : unit `| A \}, B$ to benefit from
...
\irule{SubTrans} rules ensure that our judgement builds an \emph{equivalence} and has
...
using the generic proof method developed by Geuver \cite{geuvers95}, by finding
...
\begin{theorem}[Decidability of typing]
  $`G \typea t : T$ and $`G \typea T \suba U :
...
term of type $0 \neq 0$ from the judgement $0 : \mysubset{x}{`N}{x \neq
  0}$, nor can you actually derive the latter, as it will not type-check.
...
the judgement which builds a coercion given two types (denote by $\coerce{`G}{T}{U}$). 
...
of coercions, which we do not need here as we can get unicity
...
coercions. The latter is an important property of our system, as it
...
\end{lemma}
 Finally we can show that our interpretation is correct (proposition \ref{interp-correct}).
...
$l \neq []$, but we may not have any placeholder element of type
$A$.
...
one is able to write programs with precis e specifications. However it
is not obvious if it could be made usable by programmers because of the
paradigm change and scaling issues. The \DML language \cite{Xi98:PhD} is more akin to
...
proof tools with \Coq to discharge the generated obligations. 
...
I would like to thank Christine Paulin-Mohring for directing this work and
Jean-Christophe Filli\^atre for discussions on previous versions of this paper.
