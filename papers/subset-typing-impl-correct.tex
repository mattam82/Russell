...
\begin{proof}
  ...
  il suffit de considérer la relation $"->"_{\beta\rho}$, puisque la
  relation $\teq{`G}{t}{T}{u}{U}$ est une équivalence.
  On va donc montrer que si $t "->"_{\beta\rho} u$ alors
  $\teq{`G}{t}{T}{u}{U}$.
  \begin{induction}
    \caseother{$(\lambda x : X.v)~e "->"_\beta v[e/x]$}. 
    C'est la première règle, direct.
    \caseother{$\pi_1,\pi_2$} De même.
    \casenil \TODO{passage au contexte}.
  \end{induction}
...
\begin{proof}
  ...
  \begin{induction}
    ...
    $\subimpl{`G}{c'}{V}{C}$ et $\subalgo{`G, x : C}{c_2}{B}{D}$ on a:
