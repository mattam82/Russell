...
\begin{proof}
  ...
  \begin{induction}[subtyping-decl]
    ...
    \irule{SubTrans}, des deux c\^ot\'es. On considere donc seulement
    ...
    \begin{prooftree}
      ...
      \AXC{$`G \typec c' : \hnf{T}~\subhnf \hnf{U}$}
      \UIC{$`G \typec c' : T \sub U$}
    ...
    \begin{induction}
      ...
      \begin{prooftree}
        ...
        \AXC{$`G \typec c' : \Pi x : X'.Y' \subhnf \hnf{U}$}
        \BIC{$$} %$`G \typec c = c' `o (\lambda f.\lambda x.~c_2~(f~(c_1~x))): \Pi x : X.Y \sub U$}
      \end{prooftree}
      
      Si $\subimplhnf{`G}{c'}{\Pi x : X'.Y'}{\hnf{U}}$,
      alors par induction cette derivation peut être
      réecrite en une dérivation sans application de \irule{SubTrans} qui 
      sera nécessairement de la forme:
      ...
      \begin{prooftree}
        ...
        \BIC{$\subimplhnf{`G}{\lambda f.\lambda x.d_2~(f~(d_1~x))}
          {\Pi x : X'.Y'}{\Pi x : S.T}$}
        \UIC{\vdots}
        \noLine
        \UIC{$\mathcal{D}$}
        \UIC{$`G \typec c' : \Pi x : X'.Y' \subhnf U$}
      \end{prooftree}
      
      Ici $\mathcal{D}$ représente un nombre arbitraire d'applications des
      règles \irule{SubProof} et \irule{SubHnf}.

      Par induction, il existe $e_1$, $e_2$, tels que
      $\subimpl{`G}{e_1 \eqbi c_1 `o d_1}{S}{X}$ et 
      $\subimpl{`G}{e_2 \eqbi c_2 `o d_2}{Y[e~x/x]}{T}$, donc 
      ...
      \begin{prooftree}
        \AXC{$\subimpl{`G}{c}{S}{T}$}
        \UIC{$\subimplhnf{`G}{d = (\lambda t : S.~\elt{T}{(\lambda x :
              T.P)}{(c~t)}{?_{P[c~t/x]}})}
          {\hnf{S}}{\mysubset{x}{T}{P}}$}
        %\UIC{$\subimpl{`G}{d}{S}{\mysubset{x}{T}{P}}$}
        \AXC{$\subimplhnf{`G}{e}{\mysubset{x}{T}{P}}{\hnf{U}}$}
        %\UIC{$\subimpl{`G}{e}{\mysubset{x}{T}{P}}{U}$}
        \BIC{$$} %$\subimpl{`G}{e `o d}{S}{U}$}
      \end{prooftree}
      
      Si $\subimplhnf{`G}{e}{\mysubset{x}{T}{P}}{\hnf{U}}$ alors
      $\subimpl{`G}{e'}{T}{\hnf{U}}$ est
      dérivable par une dérivation plus petite et $e = e' `o \pi_1$.
      Par induction, avec l'hypoth\`ese $\subimpl{`G}{c}{S}{T}$, il existe $f$ tel que
      $\subimpl{`G}{f}{S}{U}$ est dérivable par une dérivation
      n'utilisant pas \irule{SubTrans} et $f \eqbi e' `o c$. 
      On a bien $e `o d \eqbi e' `o \pi_1 `o d \eqbi e' `o c$ car
      \begin{eqnarray*}
        e' `o \pi_1 `o d & = & e' `o \pi_1 `o (\lambda t : S.~\elt{T}{(\lambda x :
          T.P)}{(c~t)}{?_{P[c~t/x]}}) \\
        & = &
