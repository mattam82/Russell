...
\begin{lemma}[Interprétation et conversion]
  Si $`G \typec T, T' : s$, $T "->"_{\beta\rho} T'$ alors $\ip{T}{`G} \eqbr
  \ip{T'}{`G}$.
\end{lemma}
...
\begin{proof}
  ...
  Si $T = (\lambda x : X.v)~e$, alors $T' = v[x/e]$. 
  $\ip{T}{`G} = (\lambda x : \ip{X}{`G}.\ip{v}{`G, x : X})~(c~\ip{e}{`G})$ 
  $\ip{T'}{`G} = \ip{v[x/e]}{`G}$.
\end{proof}
...
\begin{proof}
  ...
  \begin{induction}
    ...
    donc $c' = c$ dérivable par une application de \irule{SubHnf} puis \irule{SubConv}.
    ...
    $\beta\rho$-équivalents.
    
    \case{SubProd}\quad Soit $T = \Pi x : A.B$ et $U = \Pi x : C.D$.  On
    a $\subimpl{`G}{c_1}{C}{A}$ et $\subimpl{`G, x :
      C}{c_2}{B}{D}$ Or si $S \eqbr \Pi x : A.B$ et $\Pi x :
    C.D \eqbr V$ alors $\hnf{S} = \Pi x : A'.B'$ avec $A \eqbr A'$ et $B
    \eqbr B'$ et $\hnf{V} = \Pi x : C'.D'$ avec $C \eqbr C'$ et $D \eqbr
    D'$.
    
    Par induction on a $\subimpl{`G}{c_1'}{C'}{A'}$ avec $c_1' \eqbr
    c_1$ et $\subimpl{`G, x : C}{c_2}{B'}{D'}$. On peut donc
    déduire $\subimpl{`G, x : C'}{c_2'}{B'}{D'}$ avec $c_2 \eqbr c_2'$ par le lemme
    \ref{coercion-conversion-ctx-impl}.  
    On déduit par \irule{SubProd} que $\subimpl{`G}{c'}{\hnf{S}}{\hnf{V}}$ est
    dérivable. On a bien $c \eqbr c'$ car $\ip{C}{`G} \eqbr \ip{C'}{`G}$, donc 
    $\lambda x : \ip{C}{`G}.c_2~(f~(c_1~x)) \eqbr \lambda x : \ip{C'}{`G}.c_2'~(f~(c_1'~x))$.
