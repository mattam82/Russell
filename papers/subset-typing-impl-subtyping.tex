...
On définit le système $\subi$ (figure \ref{fig:subtyping-impl-rules})
...
\begin{proof}
  ...
  \begin{induction}
    ...
    \eqbi \lambda x : T.x$ dérivable par une application optionnelle de
    \irule{SubHnf} puis \irule{SubConv}. 
    C'est la seule règle applicable pour dériver
...
\begin{proof}
  ...
  \item Si $\nf{T}$ n'a pas pour symbole de tête, $\Pi, \Sigma$ ou
    ...
    type sous-ensemble auquel cas le mécanisme est similaire. 
    Dans tout les autres cas, \irule{SubConv} est la seule règle
