...
\begin{proof}
  ...
  \begin{induction}
    ...
    \case{SubProof}
    \begin{prooftree}
      \AXC{$\subalgoimpl{`G}{T}{U}$}
      \AXC{$\talgo{`G}{\mysubset{x}{U}{P}}{\Set}$}
      \BIC{$\subalgo{`G}{T}{\mysubset{x}{U}{P}}$}
    \end{prooftree}
    
    Par inversion du jugement de typage, on a $\talgo{`G}{U}{\Set}$,
    donc par application de l'hypothèse d'induction, $\talgo{`G}{T}{\Set}$.
  \end{induction}
\end{proof}
