...
\institute[]
{
  ...
  \LRI{} - \'Equipe \Demons{} \& \INRIAFuturs{} - Projet \Proval{} 
}
...
\begin{frame}
  \frametitle{Une idée simple}
  
  \begin{block}{Definition}
    $\mysubset{x}{T}{P}$ est l'ensemble des objets de l'ensemble $T$
    verifiant la propriété $P$. 
  \end{block}
  
  \alert{Sépare} objet et propriété.
...
\begin{frame}
  ...
  \begin{block}{\PVS{}}
    \vspace{0.7em}
    \begin{itemize}
    \item Typage spécialisé pour les types sous-ensemble. Génération de
...
\begin{frame}[t]
  ...
  \begin{enumerate}
  \item<1-> Un langage ``property-irrelevant'' (\lng{}) avec typage \alert{decidable} ;
  \item<2-> Une traduction totale vers \Coq{} (et termes à trous) ;
  \item<3-> Une méthode pour transformer les trous en obligations.
  \end{enumerate}
...
%      \SubSigma\DP}
  ...
  \only<6>{\begin{theorem}[Decidabilité du typage]
      $\forall\;`G\;t\;T,`G \type t : T$ est décidable.
  \end{theorem}}
\end{frame}
...
\begin{frame}
  \frametitle{\CCI{} avec métavariables}
...
\begin{frame}[t]
  \frametitle{Traduction: derivation de coercions explicites}

  \begin{block}{Correction de l'interprétation}
    \begin{center}
      Si $`G \type t : T$ alors $\ipG{`G} \typec \ip{t}{`G} : \ip{T}{`G}$.
    \end{center}
  \end{block}
      
  \begin{block}{Correction de la dérivation des coercions}
    \begin{center}
...
\begin{frame}[t]
  \frametitle{Les listes façon \Program}
  \vspace{-0.75em}
  \begin{center}%
...
\begin{frame}
  ...
  \begin{block}{Contribution}
    ...
    \alert{conservatif} par rapport à \CC{}, \alert{intégré} à l'environnement
  ...
  \begin{block}{Travaux en cours et futurs}
    \vspace{0.2em}
    \begin{itemize}
    \item Preuve \Coq{} du développement théorique.
    \item Application à d'autres constructions ((co-)inductifs) et commandes.
    \item Améliorations de \Coq{} (variables existentielles, inférence à
      la ML, proof irrelevance).
    \item Donner une interprétation relativement complete et utile des
      langages de la famille \ML{} (interprétation monadique, effets...).
    \end{itemize}
  \end{block}
