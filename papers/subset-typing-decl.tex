...
appliquer un type à un terme pour instancier une fonction polymorphe par exemple.
...
La relation $\mathcal{S}$ définissant les sommes formables
dans le système est définie inductivement par les règles suivantes:
\[\begin{array}{cccll}
  s_1 & s_2 & s_3 & \text{Habitants} & \text{Exemple} \\
  \hline
  \Prop & \Prop & \Prop & \text{Couples de propositions} & \Sigma x :  "->" x = 0  \\
  \Set & \Set & \Set & \text{Couples d'objets} & \nat * \nat \\
\end{array}
\]
...
: \Prop$ et $V : \Set$ n'ont pas d'intérêt.
