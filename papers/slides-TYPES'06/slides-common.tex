...
\begin{frame}[t]
  ...
  \begin{block}{Coercions for \PVS's \ps}
    \vspace{0.7em}
    \begin{itemize}
    \item<1-> A type system based on \ECC ($\typea, \eqbr$) with a \alert{larger} conversion ($\suba$). 
  ...
  \only<5>{
    \begin{theorem}[Conservation of the equivalence]
...
\begin{frame}[t]
  \frametitle{\Program: An example}
\end{frame}

\begin{frame}
  \frametitle{Related \& future work}
  
  \begin{block}{Related Work}
    \vspace{0.2em}
    \begin{itemize}
    \item Explicit coercions (Z. Luo, G. Chen, A. Saïbi).
    \item An alternative to \Program (C. Parent).
%    \item Works with \name{Functional Induction} (P. Courtieu).
    \end{itemize}
  \end{block}
  \pause
  ...
  \begin{block}{Further work}
    \vspace{0.2em}
    \begin{itemize}
    \item Application to other \CCI{} constructs ((co-)inductives).
