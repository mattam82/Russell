...
\begin{frame}[t]
  ...
  \begin{block}{\CCI{} with metavariables}
    \begin{center}
      \BAX{}{$`G \typec t : T$}
      {$`G \typec p : P[t/x]$}
  ...
  \pause
  \begin{theorem}[Forgetful inclusion]
    If $`G \typec t : T$ then $f(`G) \typea f(t) : f(T)$ if $f$ is
  ...
  \uncover<3->{
    \begin{definition}
      \begin{center}
        ...
        \UAX{SubConv}
        {$T \eqbr U$}
        {$\subimpl{`G}{\uncover<4->{\blue{\ctxdot}}}{T}{U}$}
        {}\DP
        
        \vspace{0.25cm}
        \uncover<5->{$\subimpl{`G}{\blue{\eltpit~\ctxdot}}{\mysubset{x}{T}{P}}{T}$}
...
\begin{frame}[t]
  ...
  \only<2->{
    ...
    \uncover<2->{\rrbracket_{`G}}\uncover<3->{ + \text{ obligations}}.\]
