...
\newcommand{\subimpl}[4]{#1 \typec #2 : #3 \suba #4}
...
\begin{figure}[ht]
  \begin{eqnarray*}
    \muimpl'~(\mysubset{x}{U}{P})   & "=>" & \letml~(f, t) = \muimpl'~(\hnf{U})~\inml~(f `o
    \pi_1, t) \\
    \muimpl'~(T)                        & "=>"  & (\sref{id}, T) \\
    \\
    \muimpl~(T) & = & \muimpl' (\hnf{T})
  \end{eqnarray*}
...
\newcommand{\BetaTeq}{
  ...
  \AXC{$T \sub U$}
  \UIC{$\teq{`G}{(\lambda x : X.v)~e}{T}{v[e/x]}{U}$}
}

\newcommand{\PiLeftTeq}{
  \AXC{$T \sub U$}
  \UIC{$\teq{`G}{\pi_1~\pair{\Sigma x : T.V}{e_1}{e_2}}{T}{e_1}{U}$}
}

\newcommand{\PiRightTeq}{
  \AXC{$T \sub U$}
  \UIC{$\teq{`G}{\pi_2~\pair{\Sigma x : V.T}{e_1}{e_2}}{T}{e_2}{U}$}
}

\newcommand{\AppTeq}{
  \AXC{$\teq{`G}{M}{S}{M'}{T} \quad \teq{`G}{N}{U}{N'}{V}$}
  \AXC{$M \sub \Pi x : A.B \quad M' \sub \Pi x : C.D$}
  \AXC{$U \sub A \quad V \sub C$}
  \TIC{$\teq{`G}{M~N}{B[N/x]}{M'~N'}{D[N'/x]}$}
}

\newcommand{\PairTeq}{
  \AXC{$\teq{`G}{e_1}{A}{e_2}{C}$}
  \AXC{$\teq{`G, x : A}{e_2}{B}{e_2'}{D}$}
  \BIC{$\teq{`G}{\pair{\Sigma x : A.B}{e_1}{e_2}}{T}{\pair{\Sigma x : C.D}{e_1'}{e_2'}}{U}$}
}

\newcommand{\PiTeq}{
  \AXC{$\teq{`G}{C}{s_1}{A}{s_1}$}
  \AXC{$\teq{`G, x : A}{B}{s_2}{D}{s_2}$}
  \BIC{$\teq{`G}{\Pi x : A.B}{s_3}{\Pi x : C.D}{s_3}$}
}

\newcommand{\SigmaTeq}{
  \AXC{$\teq{`G}{A}{s_1}{C}{s_1}$}
  \AXC{$\teq{`G, x : A}{B}{s_2}{D}{s_2}$}
  \BIC{$\teq{`G}{`S x : A.B}{s_3}{`S x : C.D}{s_3}$}
}

\newcommand{\SubsetTeq}{
  \AXC{$\teq{`G}{T}{\Set}{A}{\Set}$}
  \AXC{$\teq{`G, x : T}{P}{\Prop}{P'}{\Prop}$}
  \BIC{$\teq{`G}{\mysubset{x}{T}{P}}{\Set}{\mysubset{x}{U}{P'}}{\Set}$}
}
