...
\newcommand{\subimpl}[6]{#1 \typea #2 : #3 \sub #4 "~>" #5 \typec #6}
...
\def\AppI{
  \QAX{App}
  {$\timpl{`G}{f}{T}{`G'}{f'}{T'}$}
  {$\muterm~T' `= (\pi, \Pi x : V'. W')$}
  {$\timpl{`G}{u}{U}{`G'}{u'}{U'}$}
  {$\subimpl{`G}{u'}{U'}{V'}{`G'}{u''}$}
  {$\timpl{`G}{f u}{W[u/x]}{`G'}{(\pi~f')~u''}{W'[ u'' / x ]}$}
  {}
}
...
\def\SumInfI{
  ...
  {$\timpl{`G}{t}{T}{`G'}{t'}{T'}$}
  {$\timpl{`G}{u}{U}{`G'}{u'}{U'}$}
  {$\timpl{`G}{\Sigma T.U}{s}{`G'}{\Sigma T'.U'}{s}$}
  {$\timpl{`G}{(t, u)}{\Sigma T.U}{`G'}{(t', u')}{\Sigma T'.U'}$}
  {}
}
...
\def\SubsetI{
  ...
  {$\timpl{`G}{P}{\Pi x : U. \Prop}{`G'}{P'}{\Pi x : U'. \Prop} $}
...
\begin{figure}[h]
  ...
  \caption{Réecriture du typage vers \Coq}
\end{figure}
...
\begin{figure}
  \begin{eqnarray*}
    ...
    x                        & "=>"  & x
  \end{eqnarray*}
  \label{muterm-definition}
  \caption{Définition de $\muterm$}
\end{figure}
