...
$A$ ; ou bien d'en appliquer une (ou tout autre résultat déjâ établi): 
...
les parties codages et preuve pour simplement diviser le problème. Les
...
le lemme suivant:
(les jugements $\typea$ et $\suba$ ont la même sémantique que
dans le système déclaratif)
\begin{lemma}[Stabilité modulo $\beta$]
  Pour tout $U$, $V$, $U `=_{\beta} V "=>" U \suba V$ dérivable en
  n'appliquant \irule{SubConv} que lorsqu'aucune règle ne s'applique.
\end{lemma}

Cela signifie qu'on peut n'appliquer \irule{SubConv} que lorsque
plus aucune autre règle ne s'applique sans restreindre la conversion.
...
faisons le sous-typage dans les types,  donc les environnements (listes
de couples $(\text{nom}, \text{type})$) doivent aussi être réecrits. Cela assure aussi la
...
\item[\SubRightI\DP:] \quad\\
  ...
  des obligations qui ne seront pas prouvables.
  \vspace{1em}
