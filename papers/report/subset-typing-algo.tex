...
Notons aussi que les jugements de la forme $\talgo{`G}{t}{s}$ où $s `:
\setproptype$ sont une abréviation pour $\talgo{`G}{t}{T} `^ T
"->"\beta\rho s$, c'est une pratique courante lorsque l'on présente un
système algorithmique basé sur système de types purs.
...
\begin{lemma}[Coercion de sortes]
  ...
  Si $\subalgo{`G}{T}{s}$ où $\subalgo{`G}{s}{T}$ où $s `: \setproptype$
  alors $T \eqbr s$.
\end{lemma}
\begin{proof}
  Les seuls règles permettant de dériver de tels jugements sont
  ...
  \begin{induction}
    ...
    $\talgo{`G}{s}{\Set}$ ce qui est impossible. De façon symétrique
    pour \irule{SubSub}.
  \end{induction}
\end{proof}

\begin{lemma}[Convertibilité avec une sorte et réduction]
  \label{conv-sorts-red}
  Si $T \eqbr s$ alors $T "->"_{\beta\rho} s$.
\end{lemma}
\begin{proof}
  Par la propriété de Church-Rosser pour la réduction
  $"->"_{\beta\rho}$ il existe $v$ tel que $T "->>"_{\beta\rho} v
  "<<-"_{\beta\rho} s$. Or $s$ ne peut se réduire pas vers un autre terme, on
  a donc $T "->>"_{\beta\rho} s$.
\end{proof}

\begin{corrolary}[Coercion de sortes et réduction]
  \label{coercion-sorts-red}
  Si $\subalgo{`G}{T}{s}$ ou $\subalgo{`G}{s}{T}$ où $s `:
...
\begin{proof}
  Par induction sur mutuelle sur les dérivations de typage, coercion et bonne formation.
  ...
  \begin{induction}
    ...
    $W \redbrt s_2$. On en déduit $`G, x : S', `D \type T : s_1$ et $`G, x : S', `D
    y : T \seq U : s_2$. On applique \irule{Prod} pour obtenir 
    $`G, x : S', `D \type \Pi x : T.U : s_3$. De même, direct par induction pour le reste des règles.
       
    \casetwo{SubProd}{SubSigma} Par induction on a $\subalgo{`G, x : S', `D}{U}{T}$
    et $\subalgo{`G, x : S', `D, x : U}{V}{W}$, il suffit d'appliquer
    \irule{SubProd}. De même pour \irule{SubSigma}.

    \caseother{\irule{SubConv},\irule{SubHnf},\irule{SubProof},\irule{SubSub}} De même, direct par induction. On utilise
    l'hypothèse d'induction mutuelle pour les dérivations de typage en
    prémisse.
...
\begin{proof}
  ...
  \begin{induction}
    ...
    \ref{narrowing-a} pour la deuxième prémisse.
...
puisqu'on n'a pas de cumulativité. Par exemple on n'a pas besoin d'identifier 
$\nat : \Set$ et $(\lambda x : \Type. x) \nat : \Type$ puisque le
deuxième terme n'est pas typable dans notre système.
