...
\pgfdeclareimage[height=0.5cm]{lri-logo}{lri-logo}
\logo{\pgfuseimage{lri-logo}}

\pgfdeclareimage[height=2.5cm]{lambda-cube}{lambda-cube}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<1-> Le plus haut sommet du cube de Barendregt ; \\
    \uncover<1>{\pgfuseimage{lambda-cube}}
    \note<1>{
      \begin{description}
      ...
      \end{description}}
  \item<2-> Verbeux ;
    \note<2>{Inférence limitée (arguments implicites), 
      sous-typage peu puissant}
  \item<3-> Mêle logique et calcul.
    \note<3>{Pas de séparation nette entre calcul et preuve}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<1-> Grande richesse pour la spécification et l'implémentation ;
  \item<2-> Extraction vers du code efficace ;
  \item<3-> Distinction entre parties informatives et logiques dans
    le système de type.
  \end{itemize}
...
\begin{frame}
  ...
  \item<2-> Gang Chen: sous-typage par coercions avec types dépendants,
    conversion.
  \item<3-> Amorkare Saïbi: implémentation dans \Coq. 
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Sous-typage prédicatif}
  \begin{tabular}{lclcl}
    $t : \subset{x}{T}{P}$ & $"->"$ & $t : T$
    & $"~>"$ & $\pi_1~t$ \pause \\
    $t : T$ & $"->"$ & $t : \subset{x}{T}{P}$ 
  ...
  \note{Parler du dev. du système de type de Martin-Löf
...
\begin{frame}
  ...
  Présentation du système déclaratif. Peut-être en deux slides.
  \begin{block}{Typage}
  \end{block}
...
\begin{frame}
  ...
  \begin{itemize}
  \item
    Dévellopement d'un language de programmation 
