...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      ...
      \item Expressivité:
        \begin{itemize}[<+->]
        \item Types dépendants ;
        \item Ordre supérieur ;
        \item Types dépendants des types :
        \end{itemize}
...
%       b \neq 0 "->" \{~q : \nat~\&~\{~r : \nat `| r < b `^ a = b * q + r
%       ~\} ~\}$
      \only<2>{\def\b{{\color{red}b}}
        \def\a{{\color{red}a}}
        $\sdef{div} : \forall a : \nat, \forall \b : \nat, 
        \b \neq 0 "->" \{~q : \nat~\&~\{~r : \nat `| r < \b `^ \a = \b * q +
        r ~\} ~\}$}
      \only<3>{$\sdef{div} : \forall a : \nat, \forall b : \nat, 
        b \neq 0 "->" \{~q : \nat~\&~\{~r : \nat `| r < b `^ a = b * q +
        r ~\} ~\}$}
      

      \only<4>{$(\lambda x : \text{Value}.
...
%\PVS    Système de type intégrant le \emph{Predicate subtyping}.
\begin{frame}
