\documentclass[notheorems]{beamer} %notes=show ,handout
...
%       ~\} ~\}$
  ...
  \only<3>{
    ...
    ~|~\sdef{right} : B "->" \{A\} + \{B\}. \\
    \\
    \typeml~\sdef{sumbool} = \texttt{Left} ~|~ \texttt{Right}
  \end{array}$}
...
\begin{frame}
  ...
  \item[{\bf --}] Architecture de \PVS{} peu sûre (bugs, critère de De Bruijn).
  \end{itemize}  
\end{frame}
...
\begin{frame}[t]
  ...
  \only<1>{
    \begin{center}
      \begin{figure}
        \subfigure[Sous-typage]{\BAX{}
          {$t : A$}
          {$A \sub B$}
          {$t : B$}
          {}
          \DP}
        \subfigure[Coercions]{\BAX{}
          {$t : A$}
          {$c : A "->" B$}
          {$c~t : B$}
          {}
  ...
  \item<5-> Gang Chen: sous-typage par coercions avec types dépendants,
    conversion ;
  \item<6-> Zhaohui Luo: ``\emph{coercive subtyping}'' ; %, \pause problèmes de \alert<3>{cohérence} et \alert<4>{transitivité} ;
  \end{itemize}
  }
...
\begin{frame}[t,label=sysalgo]
  ...
  \item<3-> Algorithme de coercion dirigé par la syntaxe
    \hyperlink{subtypingdecl}{\beamergotobutton{}} ;
  \item<4->[$"=>"$] Algorithme de typage.
  \end{itemize}
...
\begin{frame}[t]
  \frametitle{Définition de la réécriture}
