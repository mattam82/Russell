...
\begin{frame}[t]
  ...
  \begin{itemize}
  \item<1-> Écriture dans un langage avec coercions implicites;
...
\begin{frame}[t,label=subtypingdecl]
  ...
  \begin{center}
    ...
    \vspace{0.5cm}
    \uncover<2->{\SubProof\DP
      \only<2>{$\nat~\sub \mysubset{x}{\nat}{x \neq 0}$}}
    
    \vspace{0.5cm}
    \uncover<3->{\SubProd\DP
      \SubSigma\DP}
  \end{center}
...
\begin{frame}[t,label=sysalgo]
  ...
  \item<3-> Algorithme de coercion déterministe (élimination de la transitivité)
    \hyperlink{subtypingdecl<2>}{\beamergotobutton{}} ;
  \end{itemize}
  ...
  \only<4>{
    \begin{theorem}[Équivalence des systèmes déclaratifs et algorithmiques]
      $`G \typed t : T$ \ssi{} il existe $U$ tel que $`G \typea t : U$ et $U \subd T$.
    \end{theorem}
...
\begin{frame}[t]
  ...
  \pause
    \begin{center}
      ...
      \vspace{0.25cm}
      \uncover<3->{\SubSubI\DP}

      \vspace{0.25cm}
      \uncover<4->{\UAX{SubProof}
      % {$`G, x : U \typec P : \Prop$}
      {$\subimpl{`G}{c}{T}{U}$}
      {$\subimpl{`G}
...
\renewcommand{\timpl}[6]{#1 \typea #2 : #3\uncover<2->{"~>" #4 \typec #5 : #6}}

\newcommand{\QAXII}[7]{\AXC{\uncover<2->{#3}}\noLine\UIC{#2}
  \AXC{\uncover<2->{#5}}\noLine\UIC{#4}
...
\begin{frame}
  ...
  \item[\bf{$"=>"$}] Travaux futurs
    \begin{itemize}
    \item
      Extension aux autres constructions de \Coq{} ((co-)inductifs \ldots) ;
    \item
      Preuve de la correction de la réécriture.
    \end{itemize}
  \end{itemize}
\end{frame}
