...
\begin{lemma}[Substitutivité de l'interprétation]
  ...
  \item $\ipG{`G, x : V} \typec \ipt{W}{\ipG{`G, x : V}} : s$ où $s `: \setproptype$.
  \item $c : \ipt{U}{\iG} \subi \ipt{V}{\iG}$ 
  \end{itemize}
  
  alors $\ipt{W}{\ipG{`G, x : V}}[c~\ipt{u}{\iG}/x] \eqbi \ipt{W[u/x]}{\iG}$.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  \begin{induction}
    ...
    $\iG{} \typec \ipt{f~u}{\iG} : \ipt{W}{\iG, x :
      \ipt{V}{\iG}}[c~u/x]$.
    On montrera enfin que $\ipt{W}{\iG, x : \ipt{V}{\iG}}[c~u/x] \eqbi
