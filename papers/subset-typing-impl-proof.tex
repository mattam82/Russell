...
\begin{proof}
  ...
  \begin{induction}
    ...
    \item Si $x "/=" y$ alors:
      ...
      pas apparaître dans une sorte). On a les propriétés:
      ...
      (lemme \ref{subi-sorts}). On en déduit la deuxième condition:
      \[\ipt{T[u/x]}{\iGr} \eqbi \ipt{T}{\iGD}\cux\]
      
      Clairement, $`a$ est l'identité car 
      \begin{eqnarray*}
        \ipT{y[u/x]}{\iGr} & = & \iGr(y) \\
        & = & \ipt{(\Gr)(y)}{\iGr} \\
        & = & \ipt{T[u/x]}{\iGr} \\
    ...
    \begin{prooftree}
      \TAX{App}
      {$`G \seq f : F \quad \mualgo(F) = \Pi y : V. W$}
      {$`G \seq e : E \quad `G \seq E, V : s$}
      {$E \sub V $}
      {$`G \seq (f e) : W [ e / y ]$}
      {}
    \end{prooftree}
    ...
    \begin{eqnarray*}
      ...
      \ipt{W[e/y][u/x]}{\iGr} & \eqbi & \ipt{W[e/y]}{\iGD}\cux
    \end{eqnarray*}
    ...
    \begin{eqnarray*}
      ...
      \ipt{W}{\iGD}[c'~\ipt{e}{\iGD}/y]) \\
      & = & ((\pi~\myipt{f})~(c'~\myipt{e})),
      \myipt{W}[c'~\myipt{e}/y]\cux \\
      \text{ où } & & \\
      \myipt{x} & = & \ipt{x}{\iGD} \\
      (\pi, \ipt{\Pi y : V.W}{\iGD}) & = & \muimpl~(\ipT{f}{\iGD}) \\
      c' & = & \sref{coerce}~\ipT{e}{\iGD}~\ipt{V}{\iGD}.
    \end{eqnarray*}
    
    Or $\ipT{e}{\iGD} = \ipt{E}{\iGD}$ par induction donc $c' :
    \ipt{E}{\iGD} \subi \ipt{V}{\iGD}$. On peut substituer pour obtenir:
    $c' : \ipt{E[u/x]}{\iGr} \subi \ipt{V[u/x]}{\iGr}$ (lemme
    \ref{subst-ipt-subi}).
    On a donc: 
    \[\ipt{(f~e)}{\iGD}\cux =
    ...
    et
    \[\ipT{(f~e)}{\iGD}\cux =
    \ipt{W}{\iGD}[c'~\ipt{e}{\iGD}/y]\cux\]
    
    
    D'autre part on a:
    \begin{eqnarray*}
      \ip{(f~e)[u/x]}{\iGr}
      & = & (\ipt{f[u/x]~e[u/x]}{\iG}, W'[c''~e'/y]) \\
      & = & ((\pi'~f')~(c''~e'), W'[c''~e'/y]) \\
      \text{ où } & & \\
      (f', T') & = & \ip{f[u/x]}{\iGr} \\
    ...
    A partir de $\mualgo(F) = \Pi y : V.W$ on peut déduire que 
    $\mualgo(F[u/x]) = \Pi y : V[u/x].W[u/x]$ par substitutivité de
    $\mualgo$ (lemme \ref{substitutive-mu}) et par le lemme
    \ref{mu-interp} on obtient \[\muimpl(\ipt{F[u/x]}{\iGr}) = 
    (\pi, \ipt{\mualgo(\Pi y : V[u/x].W[u/x])}{\iGr}) = 
    (\pi, \ipt{\Pi y : V[u/x].W[u/x]}{\iGr})\]
%     De même, à partir de $E \suba V$ on peut déduire une coercion \[c :
%     \ipt{E[u/x]}{\iGr} \subi \ipt{V[u/x]}{\iGr}\]
    
    On a donc les coercions suivantes pour la fonction $f$ et l'argument
    $e$: 
    \[
    \xymatrix{
      \ipt{F[u/x]}{\iGr}\ar[rr]_{\pi} & & \ipt{(\Pi y : V.W)[u/x]}{\iGr} \\
      \ipT{f[u/x]}{\iGr}\ar[u]^{`a_f}\ar[rr]^{\pi'} & &
      {\Pi y : V'.W'}\ar@{-->}[u]^{c_f}}
    \quad
    \xymatrix{
      \ipt{E[u/x]}{\iGr}\ar[rr]_{c'} & & \ipt{V[u/x]}{\iGr}\ar@{-->}[d]^{c_e} \\
    ...
    Pour cela on va
    montrer qu'il existe une coercion $c_f : \Pi y : V'.W' \subi 
    \ipt{\Pi z : V[u/x].W[u/x]}{\iGr}$. Cela est vrai par transitivité
    et symmétrie de la coercion. On en déduit qu'il existe $c_1, c_2$
    tels que $c_f = \lambda f.\lambda x. c_2 (f (c_1~x))$ avec
    \[
    \xymatrix{
      \ipt{V[u/x]}{\iGr}\ar[rr]^{c_1} & & V' & & W' \ar[rr]^{c_2} & & \ipt{W[u/x]}{\iGr}
    }\]
...
%     Il faut maintenant montrer que $\ipt{W[e/y][u/x]}{\iGr} \eqbi
%     \ipt{W[e/y]}{\iGD}\cux$.
